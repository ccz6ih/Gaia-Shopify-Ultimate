...
(function () {
    ...
    _updateCart: function (params) {
      return fetch(params.url, {
        ...
        .then(function (data) {
          ...
          if (data.status === 'bad_request' || data.status === 422 || data.status === 'error') {
  ...
  theme.CartForm = (function () {
    ...
    function CartForm(form) {
      ...
      this.updatingItems = {}; // Track items currently being updated to prevent concurrent updates
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      init: function () {
        ...
        // Initialize remove link handlers
        this.initRemoveLinks();
      },

      reInit: function () {
        this.initQtySelectors();
        // Note: initRemoveLinks uses event delegation, so it doesn't need to be called again
        // The handler attached in init() will work for dynamically added remove links
      },

      initRemoveLinks: function () {
        var self = this;
        // Use event delegation on document to handle remove links
        // This works even when DOM is replaced during cart rebuilds
        // Using document ensures it works even if form isn't ready yet
        if (!this.removeLinkHandlerAttached) {
          // Use capture phase to catch the event early
          document.addEventListener('click', function(evt) {
            var removeLink = evt.target.closest('.cart__remove-link');
            if (!removeLink) {
              return;
            }

            // Only handle if this remove link is within a cart form
            // Check if it's in main cart page form, drawer form, or our form
            var cartForm = removeLink.closest('#CartPageForm') ||
                          removeLink.closest('.cart__drawer-form') ||
                          (self.form && self.form.contains(removeLink) ? self.form : null);

            // If no cart form found, don't handle (might be a different remove link)
            if (!cartForm) {
              return;
            }
            ...
            // Double-check: remove href to prevent any navigation
            var originalHref = removeLink.getAttribute('href');
            if (originalHref && originalHref !== '#') {
              removeLink.setAttribute('data-original-href', originalHref);
              removeLink.setAttribute('href', '#');
            }

            // Find the cart item container (more reliable than the link)
            var cartItem = removeLink.closest('.cart__item-group[data-key]');
            if (!cartItem) {
              alert('Unable to find cart item. Please refresh the page and try again.');
              return;
            }

            // Get key from cart item container (more reliable source)
            var key = cartItem.getAttribute('data-key');

            // Fallback: try to get from link's data-key attribute
            if (!key) {
              key = removeLink.getAttribute('data-key');
            }

            // Fallback: try to extract from data-original-href if we saved it
            if (!key && removeLink.getAttribute('data-original-href')) {
              var hrefMatch = removeLink.getAttribute('data-original-href').match(/[?&]id=([^&]+)/);
              if (hrefMatch) {
                key = decodeURIComponent(hrefMatch[1]);
              }
            }

            // Final fallback: try current href (should be # but just in case)
            if (!key && removeLink.href && removeLink.href !== '#' && removeLink.href.indexOf('#') === -1) {
              var hrefMatch = removeLink.href.match(/[?&]id=([^&]+)/);
              if (hrefMatch) {
                key = decodeURIComponent(hrefMatch[1]);
              }
            }
            ...
            if (!key || typeof key !== 'string' || key.trim() === '') {
              alert('Unable to remove item: invalid item identifier. Please refresh the page and try again.');
              return;
            }
            ...
            // Get previous quantity and IDs before removal
            var previousQty = self.previousQuantities[key] || 0;
            var variantId = null;
            var productId = null;
            if (cartItem) {
              var qtyInput = cartItem.querySelector('.js-qty__num');
              if (qtyInput) {
                previousQty = parseInt(qtyInput.value) || previousQty;
              }
              // Get variant_id and product_id from data attributes using getAttribute
              var variantIdAttr = cartItem.getAttribute('data-variant-id');
              var productIdAttr = cartItem.getAttribute('data-product-id');
              if (variantIdAttr) {
                variantId = parseInt(variantIdAttr, 10);
              }
              if (productIdAttr) {
                productId = parseInt(productIdAttr, 10);
              }
            }

            // Validate key one more time before API call
            ...
            removeLink.style.cursor = 'wait';

            theme.cart.changeItem(key, 0)
              .then(function(cart) {
                // Fire product_removed_from_cart event for Emarsys IMMEDIATELY
                // This must happen before buildCart() to ensure Emarsys gets the update right away
                if (previousQty > 0) {
                  document.dispatchEvent(new CustomEvent('product_removed_from_cart', {
                    detail: {
                ...
                // Fire Emarsys cart command with updated cart (including empty cart)
                ...
                // Update previousQuantities before buildCart
                cart.items.forEach(function(item) {
                  self.previousQuantities[item.key] = item.quantity;
                });
                // Remove deleted item from tracking
                delete self.previousQuantities[key];

                // Rebuild cart UI after events are fired
                self.buildCart();
              ...
              .catch(function(error) {
                ...
                // Rebuild cart to restore state
                self.buildCart();
              });
          }, true); // Use capture phase
          this.removeLinkHandlerAttached = true;
        }
      },
      ...
      cartMarkup: function (text) {
        ...
        /* Update totals in carts when the user changes a quantity */
        updateCartMessages()

        this.reInit();

        // Initialize previousQuantities from current cart state
        // Get quantities from the DOM after cart is rebuilt
        var qtyInputs = this.form.querySelectorAll('.js-qty__num');
        var self = this;
        qtyInputs.forEach(function(input) {
          var key = input.dataset.id;
          if (key) {
            self.previousQuantities[key] = parseInt(input.value) || 0;
          }
        });
      ...
      quantityChanged: function (evt) {
        ...
        // Validate key format (should be a string/number, not empty)
        ...
        if (this.updatingItems[key]) {
          console.warn('Cart item ' + key + ' is already being updated, skipping duplicate request');
          return;
        }

        // Mark this item as being updated
        this.updatingItems[key] = true;

        // Get variant_id and product_id from DOM before removal (in case item is removed)
        var cartItemBeforeUpdate = this.form.querySelector('[data-key="' + key + '"]');
        var variantIdBeforeUpdate = null;
        var productIdBeforeUpdate = null;
        if (cartItemBeforeUpdate) {
          var itemData = cartItemBeforeUpdate.dataset;
          if (itemData.variantId) {
            variantIdBeforeUpdate = parseInt(itemData.variantId);
          }
          if (itemData.productId) {
            productIdBeforeUpdate = parseInt(itemData.productId);
          }
        }
        ...
        theme.cart.changeItem(key, qty)
          .then(function (cart) {
            ...
            const previousQty = this.previousQuantities[key] || 0;
            ...
            // Dispatch Shopify events for Emarsys IMMEDIATELY
            // This must happen before buildCart() to ensure Emarsys gets the update right away
            if (updatedItem) {
              const qtyDiff = updatedItem.quantity - previousQty;

              if (qtyDiff > 0) {
                // Quantity increased - fire product_added_to_cart
                ...
                // Quantity decreased - fire product_removed_from_cart
                document.dispatchEvent(new CustomEvent('product_removed_from_cart', {
                  detail: {
                    variant_id: updatedItem.variant_id,
                    product_id: updatedItem.product_id,
              ...
              // Use variant_id and product_id captured before removal
              document.dispatchEvent(new CustomEvent('product_removed_from_cart', {
                detail: {
                  variant_id: variantIdBeforeUpdate,
                  product_id: productIdBeforeUpdate,
                  quantity: previousQty,
                  cart: cart
                }
              }));
            ...
            // Dispatch custom cart:updated event (for your discount logic)
            document.dispatchEvent(new CustomEvent('cart:updated', {
              detail: {
                cart: cart
              }
            ...
            // Store current quantities for next comparison
            cart.items.forEach(function(item) {
              this.previousQuantities[item.key] = item.quantity;
            }.bind(this));
            // Remove deleted item from tracking if it's gone
            if (!updatedItem && previousQty > 0) {
              delete this.previousQuantities[key];
            }
          ...
          .catch(function (error) {
            ...
            // Rebuild cart to get fresh state
            this.buildCart();
          }.bind(this))
          .finally(function() {
            // Always clear the updating flag, even on error
            delete this.updatingItems[key];
          }.bind(this));
      },
  ...
  theme.AjaxProduct = (function () {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      ...
      success: function (product) {
        ...
        // Dispatch Shopify standard event for Emarsys
        document.dispatchEvent(new CustomEvent('product_added_to_cart', {
          detail: {
  ...
  class NewsletterReminder extends HTMLElement {
    constructor() {
      ...
      // Make the text trigger work with keyboard input
      if (this.popupTrigger) {
        this.popupTrigger.addEventListener('keydown', (event) => {
          if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault(); // Prevent scrolling when hitting space
            this.popupTrigger.click(); // Trigger the modal
          }
        });
      }
    }
    ...
    show(time = this.secondsBeforeShow, forceOpen = false) {
      const reminderAppeared = (window.localStorage.getItem('reminderAppeared') === 'true');

      if (!reminderAppeared) {
        setTimeout(() => {
          this.dataset.enabled = 'true';
          window.localStorage.setItem('reminderAppeared', true);
        }, time * 1000);
      }
    }
  ...
  class ImageElement extends HTMLElement {
    ...
    init() {
      ...
      */
      ...
      // Mobile-specific fix: Make images visible immediately on mobile
  ...
  theme.HeaderCart = (function () {
    ...
    HeaderCart.prototype = Object.assign({}, HeaderCart.prototype, {
      ...
      open: function (evt) {
        ...
        // Move focus to the cart drawer
        setTimeout(() => {
          const button = document.getElementById('cart_drawer_checkout_button');
          if (button) {
            button.focus();
          } else {
  ...
  */
  theme.buildProductGridItem = function (items, imageSize, imageFill) {
    ...
    /*
      Check global image breathing room setting for products
      Apply to product grid item markup
    */

    let imageBreathingRoom;
    if (theme.settings.imageBreathingRoom === 0) {
      imageBreathingRoom = 'false';
    } else {
      imageBreathingRoom = 'true';
    }

    items.forEach(product => {
      var image = theme.buildProductImage(product, imageSize, imageFill);
      var markup = `
        <div class="grid-item grid-product grid-product-image-breathing-room--${imageBreathingRoom}">
    ...
    Map.prototype = Object.assign({}, Map.prototype, {
        ...
        if (apiStatus === 'loaded') {
          ...
          if (apiStatus !== 'loading') {
            ...
            if (typeof window.google === 'undefined' || typeof window.google.maps === 'undefined') {
              ...
              script.src = 'https://maps.googleapis.com/maps/api/js?key=' + this.key; // + '&region=US&language=en';
      ...
      createMap: function () {
        ...
        return geolocate(mapDiv)
          .then(
            function (results) {
              var mapZoom = parseInt(mapDiv.getAttribute('data-map-zoom')),
                  mapDraggable = mapDiv.getAttribute('data-map-draggable'),
                  mapSimplified = mapDiv.getAttribute('data-map-simplified'),
                  mapStyle = mapDiv.getAttribute('data-map-style');
              var mapOptions = {
                zoom: mapZoom,
                ...
                styles: mapStyles[mapSimplified].concat(mapStyles[mapStyle])
              };
  ...
  theme.NewsletterPopup = (function () {
    ...
    NewsletterPopup.prototype = Object.assign({}, NewsletterPopup.prototype, {
      initPopupDelay: function () {
        ...
        setTimeout(function () {
          const newsletterAppeared = (window.localStorage.getItem('newsletterAppeared') === 'true');
          if (newsletterAppeared) {
            ...
            this.modal.open();
            window.localStorage.setItem('newsletterAppeared', true);
          }
  ...
  class CountdownTimer extends HTMLElement {
    ...
    _calculate() {
      ...
      const timeDifference = +new Date(`${this.month}/${this.day}/${this.year} ${this.hour}:${this.minute}:00 UTC-07:00`).getTime() - +new Date().getTime();
  ...
  theme.Product = (function () {
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      /**
       * This method is called when the user changes the variant on the product details
       * It needs to update the price information held in price.liquid
       * It assumes that price.liquid is using display_unified pricing
       * @param {*} evt
       */
      updatePrice: function (evt) {
        ...
        if (variant) {
          ...
          var priceSpan = jQuery(this.cache.price).find('.product-price')
          var globalDiscountPercent = priceSpan.attr('data-global-discount-percent') / 100.0
          jQuery(priceSpan.attr('data-regular-price')).val(variant.price)
          var globalDiscountPrice = variant.price * (1.0 - globalDiscountPercent)
          var globalDiscountAmount = variant.price - globalDiscountPrice
          jQuery(priceSpan.attr('data-global-discount-price')).val(globalDiscountPrice)
          var formattedPrice = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat)
          var priceHtml = 'Original: ' + formattedPrice
          if (globalDiscountPercent > 0.0) {
            var formattedDiscountPrice = theme.Currency.formatMoney(globalDiscountPrice, theme.settings.moneyFormat)
            var formattedDiscountAmount = theme.Currency.formatMoney(globalDiscountAmount, theme.settings.moneyFormat)
            priceHtml += ' <br /> Gaia Members: ' + formattedDiscountPrice + ' (Save ' + formattedDiscountAmount + ')'
          }

          // suppress zeros when applicable
          if (variant.price > 100000 && variant.price % 100 === 0.0 && globalDiscountPrice % 100 === 0.0) {
            var formattedPriceNoZeros = theme.Currency.formatMoney(variant.price, theme.settings.moneyFormat)
            priceHtml = 'Original: ' + formattedPriceNoZeros
            if (globalDiscountPercent > 0.0) {
              var formattedDiscountPriceNoZeros = theme.Currency.formatMoney(globalDiscountPrice, theme.settings.moneyFormat)
              var formattedDiscountAmountNoZeros = theme.Currency.formatMoney(globalDiscountAmount, theme.settings.moneyFormat)
              priceHtml += ' <br /> Gaia Members: ' + formattedDiscountPriceNoZeros + ' (Save ' + formattedDiscountAmountNoZeros + ')'
            }
          }

          priceSpan[0].innerHTML = '<span>' + priceHtml + '</span>'
...
$("a:link").css({ 'outline' : '', 'text-decoration' : '' });
